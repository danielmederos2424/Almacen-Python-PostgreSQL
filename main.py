...
class MainWindow(customtkinter.CTk):
    def __init__(self):
        ...
        self.lista_almacenes = customtkinter.CTkScrollableFrame(self, label_text="Lista de Almacenes", width=650, height=300)
    ...
    def listar_almacenes(self):
        ...
        for i, config_file in enumerate(config_files):
            ...
            almacen_label = customtkinter.CTkLabel(master=almacen_frame, text=almacen_name,fg_color="#505050", corner_radius=6, font=customtkinter.CTkFont(size=15, weight="bold"))
            ...
            self.connect_button = customtkinter.CTkButton(master=button_frame, text="Conectar", fg_color="#28a745", hover_color="dark green", command=lambda almacen_name=almacen_name: connect_button_event(almacen_name))
            self.connect_button.grid(row=0, column=0, padx=5)
            self.delete_button = customtkinter.CTkButton(master=button_frame, text="Eliminar", fg_color="#dd1327", hover_color="dark red", command=lambda almacen_name=almacen_name: delete_button_event(self, almacen_name))
                ...
                if os.path.exists(config_file_path):
                    with open(config_file_path, "r") as file:
                        ...
                        try:
                            ...
                            success_label = customtkinter.CTkLabel(master=success_window, text=success_text, wraplength=300, font=customtkinter.CTkFont(size=15))
                            success_label.pack()
                            CTkButton(master=success_window, text="ACCEDER", fg_color="#28a745", hover_color="dark green", command=lambda:cargar_db(success_window, connection)).pack(expand=True, pady=(10,5), padx=20)
                            ...
                            error_label = customtkinter.CTkLabel(master=error_window, text=error_text, wraplength=300, font=customtkinter.CTkFont(size=15))
                            error_label.pack()
                            CTkButton(master=error_window, text="REINTENTAR", command=lambda:connect_error_button_event()).pack(expand=True, pady=10, padx=20)
        ...
        self.logo_label = customtkinter.CTkLabel(self.sidebar_frame, text="ALMACENES", font=customtkinter.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))
        self.update_button = customtkinter.CTkButton(self.sidebar_frame, text="ACTUALIZAR LISTA", command=self.update_button_event)
...
def cargar_db(success_window, connection):   
    ...
    option_label = customtkinter.CTkLabel(master=sidebar_frame, text="SELECCIONE UNA OPCIÓN: ", font=customtkinter.CTkFont(size=15))
    option_label.pack(side="top", padx=20, pady=(10, 10))
    update_button = customtkinter.CTkButton(master=sidebar_frame, text="ACTUALIZAR LISTADO", command=lambda: update_product_button_event())
    update_button.pack(side="top", padx=20, pady=10)
    button1 = customtkinter.CTkButton(master=sidebar_frame, text="NUEVA CATEGORIA", command=lambda: category_button_event(cursor))
    button1.pack(side="top", padx=20, pady=10)
    button2 = customtkinter.CTkButton(master=sidebar_frame, text="NUEVO PRODUCTO", command=lambda: new_product_button_event(cursor))
    button2.pack(side="top", padx=20, pady=10)
    button3 = customtkinter.CTkButton(master=sidebar_frame, text="ABASTECER PRODUCTO", command=lambda:refill_product_button_event(cursor))
    button3.pack(side="top", padx=20, pady=10)
    button4 = customtkinter.CTkButton(master=sidebar_frame, text="REGISTRAR COMPRA",fg_color="#28a745", hover_color="dark green", command=lambda:registrar_compra(cursor))
    button4.pack(side="top", padx=20, pady=10)
    button5 = customtkinter.CTkButton(master=sidebar_frame, text="HISTORIAL DE COMPRAS",fg_color="#28a745", hover_color="dark green", command=lambda:historial_compras(cursor))
    ...
    filter_label = customtkinter.CTkLabel(master=sidebar_frame, text="ORDENAR PRODUCTOS: ", font=customtkinter.CTkFont(size=13))
    ...
    def registrar_compra(cursor):
        ...
        CTkLabel(master=registrar_compra, text="INGRESE LOS DETALLES DE LA COMPRA", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").pack(expand=True, pady=(15, 5), padx=(20, 20))   
        ...
        CTkButton(master=registrar_compra, text="PROCEDER", fg_color="#28a745", hover_color="dark green", command=lambda: registrar_compra_button(cursor, nombre_producto.get(), cantidad_compra.get(), nombre_cliente.get())).pack(expand=True, pady=5, padx=20) # type: ignore
        ...
        def registrar_compra_button(cursor, nombre_producto, cantidad_compra, nombre_cliente):
            try: 
                nombre_producto_compra, cantidad_compra, nombre_cliente = nombre_producto, int(cantidad_compra), nombre_cliente

                cursor.execute("SELECT precio_unidad, cantidad_stock FROM PRODUCTOS WHERE nombre_producto = %s", (nombre_producto_compra,))
                ...
                """, (nombre_producto_compra, cantidad_compra, precio_unidad, precio_total_compra, nombre_cliente, fecha_compra, hora_compra))
                ...
                photo = ImageTk.PhotoImage(success_image)
                success_label = customtkinter.CTkLabel(master=success_window, text="", image=photo) # type: ignore
                success_text = "INFORMACIÓN DE LA COMPRA: \n\n" + "Producto: " + nombre_producto_compra + "\n" + "Cantidad: " + str(cantidad_compra) + "\n" + "Precio por unidad: " + str(precio_unidad) + "\n" + "Precio total: " + str(precio_total_compra) + "\n" + "Cliente: " + nombre_cliente + "\n" + "Fecha: " + str(fecha_compra) + "\n" + "Hora: " + str(hora_compra) + "\n\n" + "La compra ha sido registrada con éxito."
                success_label = customtkinter.CTkLabel(master=success_window, text=success_text, wraplength=300, font=customtkinter.CTkFont(size=15), pady=30) # type: ignore
                success_label.pack()
                ...
                # Mostrar mensaje de éxito
                success_label.pack()
                CTkButton(master=success_window, text="CONTINUAR", fg_color="#28a745", hover_color="dark green", command=lambda:continuar(cursor)).pack(expand=True, pady=(10,5), padx=20)
                ...
                error_label = customtkinter.CTkLabel(master=error_window, text=error_text, wraplength=300, font=customtkinter.CTkFont(size=15))
                error_label.pack()
                CTkButton(master=error_window, text="REINTENTAR", command=lambda:reintentar()).pack(expand=True, pady=10, padx=20)
                def reintentar():
                    error_window.destroy()             
    
    def historial_compras(cursor):
        ...
        column_names = ["ID", "Producto", "Cantidad", "Precio por unidad", "Precio total", "Nombre cliente", "Fecha", "Hora"]
        table_data = [list(compra) for compra in compras]
        table_data.insert(0, column_names)
        productos_frame = customtkinter.CTkScrollableFrame(master=historial_compras, label_text="COMPRAS", width=900, height=500)
    ...
    def check_stock (cursor):
        ...
        if productos:
            ...
            check_stock.resizable(0,0) #type: ignore
            ...
            image_label.image = photo # type: ignore
            image_label.pack()
            message = ("Los siguientes productos se encuentran escasos en stock. Por favor, reabastézcalos lo antes posible:")
            message_label = customtkinter.CTkLabel(master=check_stock, text=message, wraplength=300, font=customtkinter.CTkFont(size=15), pady=10)
            message_label.pack()
            message2 = "\n".join(producto[0] for producto in productos)
            message_label2 = customtkinter.CTkLabel(master=check_stock, text=message2, wraplength=300, font=customtkinter.CTkFont(size=15), pady=10)
            message_label2.pack()
    ...
    # Sección mostrar productos
        column_names = ["ID", "Categoría", "Nombre del producto", "Cantidad en stock", "Precio por unidad", "Fecha de vencimiento"]
        table_data = [list(producto) for producto in productos]
        table_data.insert(0, column_names)
        productos_frame = customtkinter.CTkScrollableFrame(master=db_window, label_text="PRODUCTOS", width=900, height=500)
    ...
    def refill_product_button_event(cursor):
        #Ventana para reabastecer un producto
        refill_product = customtkinter.CTkToplevel()
        refill_product.geometry("300x300")
        refill_product.resizable(0,0) #type: ignore
        refill_product.title("ABASTECER PRODUCTO EXISTENTE")
        
        CTkLabel(master=refill_product, text="ABASTECER PRODUCTO", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").pack(expand=True, pady=(15, 5), padx=(20, 20))   
        ...
        CTkButton(master=refill_product, text="ABASTECER", fg_color="#28a745", hover_color="dark green", command=lambda: refill_button(cursor, product_id.get(), cantidad.get())).pack(expand=True, pady=5, padx=20) # type: ignore
        CTkButton(master=refill_product, text="CANCELAR", fg_color="#dd1327", hover_color="dark red", command=refill_product.destroy).pack(expand=True, pady=5, padx=20)
    ...
    def category_button_event(cursor):
        #Ventana para insertar una nueva categoría
        new_category = customtkinter.CTkToplevel()
        new_category.geometry("300x300")
        new_category.resizable(0,0) #type: ignore
        new_category.title("INSERTAR NUEVA CATEGORÍA")

        
        CTkLabel(master=new_category, text="NUEVA CATEGORÍA", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").pack(expand=True, pady=(15, 5), padx=(20, 20))   
        ...
        CTkButton(master=new_category, text="GUARDAR CATEGORÍA", fg_color="#28a745", hover_color="dark green", command=lambda: guardar_categoria_button(cursor, nombre_categoria.get())).pack(expand=True, pady=5, padx=20) # type: ignore
        CTkButton(master=new_category, text="CANCELAR", fg_color="#dd1327", hover_color="dark red", command=new_category.destroy).pack(expand=True, pady=5, padx=20)
        
        def guardar_categoria_button(cursor, nombre_categoria):
            try: 
    ...
    def new_product_button_event(cursor): 
        #Ventana para insertar un nuevo producto
        new_product = customtkinter.CTkToplevel()
        new_product.geometry("600x400")
        new_product.resizable(0,0) #type: ignore
        new_product.title("INSERTAR NUEVO PRODUCTO")    


        CTkLabel(master=new_product, text="INGRESE LOS DETALLES DEL PRODUCTO", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").pack(expand=True, pady=(15, 5), padx=(20, 20))   
        ...
        fecha_vencimiento = CTkEntry(master=new_product, placeholder_text="Fecha de vencimiento (YYYY-MM-DD)", width=300)
        fecha_vencimiento.pack(expand=True, pady=5, padx=20)
        CTkButton(master=new_product, text="GUARDAR PRODUCTO", fg_color="#28a745", hover_color="dark green", command=lambda: guardar_producto_button(cursor, nombre_producto.get(), categoria.get(), cantidad_stock.get(), precio_unidad.get(), fecha_vencimiento.get())).pack(expand=True, pady=5, padx=20) # type: ignore
        CTkButton(master=new_product, text="CANCELAR", fg_color="#dd1327", hover_color="dark red", command=new_product.destroy).pack(expand=True, pady=5, padx=20)
        
        def guardar_producto_button(cursor, nombre_producto, categoria, cantidad_stock, precio_unidad, fecha_vencimiento):
            try: 
...
def delete_button_event(self, almacen_name):
    ...
    if os.path.exists(config_file_path):
        ...
        delete_success.resizable(0,0) #type: ignore
        ...
        CTkLabel(master=delete_success, text="ALMACÉN ELIMINADO", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").place(relx=0.5, rely=0.3, anchor=customtkinter.CENTER)   
...
def new_button_event():
    ...
    new_window.resizable(0,0) #type: ignore
    ...
    CTkLabel(master=nuevo, text="INGRESE LOS PARÁMETROS DE CONEXIÓN DEL SERVIDOR", font=customtkinter.CTkFont(size=18, weight="bold"), justify="center").pack(expand=True, pady=(15, 5), padx=(20, 20))   
    CTkLabel(master=nuevo, text="Debe conectarse a una base de datos previamente creada la cual no contenga información almacenada o podrían ocurrir errores. Proceder con precaución...", wraplength=500, font=customtkinter.CTkFont(size=15, weight="normal"), justify="center").pack(expand=True, pady=(0, 15), padx=(25, 25))   
    ...
    CTkButton(master=nuevo, text="PROBAR CONEXIÓN", fg_color="#28a745", hover_color="dark green", command=lambda: connect_button_event_new(name.get(), host.get(), database.get(), user.get(), password.get())).pack(expand=True, pady=5, padx=20) # type: ignore
    CTkButton(master=nuevo, text="CANCELAR", fg_color="#dd1327", hover_color="dark red", command=new_window.destroy).pack(expand=True, pady=5, padx=20)
    

    #Intentar conexión con la base de datos
    def connect_button_event_new(name, host, database, user, password):
        try:             
            connection = psycopg2.connect(
                host=host,
                database=database,
            ...
            success_label = customtkinter.CTkLabel(master=success_window, text=success_text, wraplength=300, font=customtkinter.CTkFont(size=15))
            success_label.pack()
            CTkButton(master=success_window, text="GUARDAR", fg_color="#28a745", hover_color="dark green", command=lambda:connect_success_button_event()).pack(expand=True, pady=(10,5), padx=20)
            CTkButton(master=success_window, text="NO GUARDAR", fg_color="#dd1327", hover_color="dark red", command=lambda:connect_success_cancel_button_event()).pack(expand=True, pady=(5,10), padx=20)
            new_window.destroy()
