...
def cargar_db(success_window, connection):
    ...
    button4 = customtkinter.CTkButton(master=sidebar_frame, text="REGISTRAR COMPRA", fg_color="#28a745",
                                      hover_color="dark green", command=lambda: registrar_compra(cursor))
    ...
    button5 = customtkinter.CTkButton(master=sidebar_frame, text="HISTORIAL DE COMPRAS", fg_color="#28a745",
                                      hover_color="dark green", command=lambda: historial_compras(cursor))
    ...
    def registrar_compra(cursor):
        ...
        CTkButton(master=registrar_compra, text="PROCEDER", fg_color="#28a745", hover_color="dark green",
                  command=lambda: registrar_compra_button(cursor, nombre_producto.get(), cantidad_compra.get(),
                                                          nombre_cliente.get())).pack(expand=True, pady=5,
                                                                                      padx=20)  # type: ignore
        CTkButton(master=registrar_compra, text="CANCELAR", fg_color="#dd1327", hover_color="dark red",
                  command=registrar_compra.destroy).pack(expand=True, pady=5, padx=20)
    ...
    def category_button_event(cursor):
        ...
        CTkButton(master=new_category, text="GUARDAR CATEGORÍA", fg_color="#28a745", hover_color="dark green",
                  command=lambda: guardar_categoria_button(cursor, nombre_categoria.get())).pack(expand=True, pady=5,
                                                                                                 padx=20)  # type: ignore
    ...
    def new_product_button_event(cursor):
        ...
        CTkButton(master=new_product, text="GUARDAR PRODUCTO", fg_color="#28a745", hover_color="dark green",
                  command=lambda: guardar_producto_button(cursor, nombre_producto.get(), categoria.get(),
        ...
        def guardar_producto_button(cursor, nombre_producto, categoria, cantidad_stock, precio_unidad,
                                    fecha_vencimiento):
            try:
...
def new_button_event():
    ...
    CTkButton(master=nuevo, text="PROBAR CONEXIÓN", fg_color="#28a745", hover_color="dark green",
              command=lambda: connect_button_event_new(name.get(), host.get(), database.get(), user.get(),
